...
try:
    with open(b, "r") as f:
        s = f.readlines()
        SIZE = WIDTH, HEIGHT = len(s[0]) * 50 - 50, len(s) * 50
    STEP = 50
    ...
    player, level_x, level_y = generate_level(load_level(b))
            ...
            if event.type == pygame.QUIT:
                ...
                with open(b) as f:
                    if event.key == pygame.K_LEFT:
                        if player.rect.x > 49 and f.readlines()[player.rect.y // 50][(player.rect.x - 16) // 50 - 1] != "#":
                            player.rect.x -= STEP
                    if event.key == pygame.K_RIGHT:
                        if player.rect.x < WIDTH - 50 and f.readlines()[player.rect.y // 50][(player.rect.x - 16) // 50 + 1] != "#":
                            player.rect.x += STEP
                    if event.key == pygame.K_UP:
                        if player.rect.y > 5 and f.readlines()[player.rect.y // 50 - 1][(player.rect.x - 16) // 50] != "#":
                            player.rect.y -= STEP
                    if event.key == pygame.K_DOWN:
                        if player.rect.y < HEIGHT - 50 and f.readlines()[player.rect.y // 50 + 1][(player.rect.x - 16) // 50] != "#":
                            player.rect.y += STEP
